...
router.post("/validateanswer", async (req, res) => {
    ...
    try {
        ...
        const { validatedAnswer, totalMarks } = await validateQuizAnswer(req.body)
        ...
        if (!reqUserQuiz) {
            ...
            if (userQuiz.status === "Open") {
                ...
                const ress = await QuizResponse.create({
                    quizId, student, answer, validatedAnswer, quizDuration, submittedInSec, totalMarks
                })
                ...
                res.status(200).json({
                    ...
                    value: { type: "result", message: "Your Responce Submitted", studentQuizDetails: totalMarks, quizId, submittedInSec }
        ...
        if (reqUserQuiz) {
            if (reqUserQuiz.student.email === student.email) {
                return res.status(409).json({
                    ...
                    value: {
                        message: "Email already exists OR Already Responded!", type: "error", studentQuizDetails: totalMarks, quizId, submittedInSec
                    }
                });
            }
        }
